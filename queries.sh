...
echo "$($PSQL "SELECT COALESCE(SUM(winner_goals), 0) FROM games")"

echo -e "\nTotal number of goals in all games from both teams combined:"
echo "$($PSQL "SELECT COALESCE(SUM(winner_goals + opponent_goals), 0) FROM games")"

echo -e "\nAverage number of goals in all games from the winning teams:"
echo "$($PSQL "SELECT COALESCE(AVG(winner_goals), 0) FROM games")"

echo -e "\nAverage number of goals in all games from the winning teams rounded to two decimal places:"
echo "$($PSQL "SELECT COALESCE(ROUND(AVG(winner_goals)::numeric, 2), 0) FROM games")"

echo -e "\nAverage number of goals in all games from both teams:"
echo "$($PSQL "SELECT COALESCE(AVG(winner_goals + opponent_goals), 0) FROM games")"

echo -e "\nMost goals scored in a single game by one team:"
echo "$($PSQL "SELECT COALESCE(MAX(GREATEST(winner_goals, opponent_goals)), 0) FROM games")"

echo -e "\nNumber of games where the winning team scored more than two goals:"
echo "$($PSQL "SELECT COALESCE(COUNT(*), 0) FROM games WHERE winner_goals > 2")"
